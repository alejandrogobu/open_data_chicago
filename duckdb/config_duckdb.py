...
with duckdb.connect(f'{duckdb_root}/chicago_crimes.db') as con:
    ...
    try:
        ...
        CREATE OR REPLACE TABLE RAW.CRIMES AS 
        SELECT * FROM READ_PARQUET('r2://{bucket_raw}/chicago_crimes/crimes/**/*.parquet', filename=true);
        """
        ...
        count_row_query = "SELECT count(*) FROM RAW.CRIMES;"
        start_time = time.time()
        count_row = con.execute(count_row_query).fetchdf()  # .fetchdf() to directly retrieve as DataFrame
        finish_time = time.time()
        elapsed_time = finish_time - start_time
        print(f"Time taken to count rows: {elapsed_time} seconds")
